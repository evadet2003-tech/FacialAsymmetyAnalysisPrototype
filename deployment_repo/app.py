...
def download_model():
    ...
    if not os.path.exists(model_path) or os.path.getsize(model_path) < 1000000:
        with st.spinner("Downloading face detection model... This only happens once."):
            try:
...
if MODEL_PATH is None:
    st.error("Could not load the face detection model. Please refresh the page to try again.")
    st.stop()
...
#POINT OVERLAY
    ...
    if st.session_state.image is None:
        st.warning("No image found. Please go back and upload an image.")
        ...
        try:
            ...
            with vision.FaceLandmarker.create_from_options(options) as landmarker:
                ...
                img_for_detection = np.ascontiguousarray(st.session_state.image)
                mp_image = mp.Image(image_format=mp.ImageFormat.SRGB, data=img_for_detection)
                ...
                if result.face_landmarks:
                    ...
                    st.error("Face detection failed. Please ensure the image contains a clearly visible face.")
                    if st.button("Back to Upload"):
                        st.session_state.step = 1
                        st.rerun()
        except Exception as e:
...
#INTERACTIVE 3D
    ...
    if st.session_state.coords_3d is None or st.session_state.image is None:
        st.warning("No analysis data found. Please start from the beginning.")
        if st.button("Back to Upload"):
            st.session_state.step = 1
            st.rerun()
    else:
...
#REPORT
    ...
    if st.session_state.analysis_results is None:
        st.warning("No analysis results found. Please start from the beginning.")
        if st.button("Back to Upload"):
            st.session_state.step = 1
            st.rerun()
    else:
